...
entity pipeline_sans_alea is 
  port(
    ...
    RAZ_DI: in std_logic;
    Clr_EX: in std_logic;
    EA_EX: in std_logic_vector(1 downto 0);
    EB_EX: in std_logic_vector(1 downto 0);
    a1, a2, rs1, rs2, CC, op3_EX_out, op3_ME_out, op3_RE_out: out std_logic_vector(3 downto 0)
  );
end entity;

architecture pipeline_arch of pipeline_sans_alea is
    signal ALUSrc_EX, MemWr_Mem, MemWr_RE, PCSrc_ER, Bpris_EX, RegWR, MemToReg_RE : std_logic;
    signal RegSrc, immSrc, ALUCtrl_EX : std_logic_vector(1 downto 0);

    signal instr_DE: std_logic_vector(31 downto 0);

    signal PCSrc_0, PCSrc_1, PCSrc_2, PCSrc_3: std_logic;
    signal CCWr, CCWr_1 : std_logic;
    signal RegWR_0, RegWR_1, RegWR_2, RegWR_3 : std_logic;
...
begin
    ...
    Unite_cond: entity work.cond
      ...
      RegWr_2 <= RegWr_1 & CondEx_0(0);
      PCSrc_2 <= MemToReg_1 & CondEx_0(1);
      MemWr_2 <= MemWr_1 & CondEx_0(2);
      Branch_2 <= Branch_1 & CondEx_0(3);
    ...
    Bascule_MemToReg_4: entity work.Reg1
      port map(
        source => MemToReg_3,
        output => MemToReg_4,
        wr => '1',
    ...
    Proc:entity work.proc
      port map(
        ...
        Clr_EX => Clr_EX, 
        MemToReg_RE => MemToReg_4, -- UCtrl/UCond
